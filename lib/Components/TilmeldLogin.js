...
var TilmeldLogin = (function(Nymph, User) { "use strict";
  ...
  return {
    ...
    compactText: 'Login/Register', // The text used to toggle the dialog when 'compact' layout is selected.
    ...
    existingUser: false, // This determines whether the 'I have an account.' box is checked and the registration form is shown.
  ...
  openAccountRecovery() {
    // TODO(hperrin): Implement an AJAX account recovery.
  ...
  this.observe('username', (newValue, oldValue) => {
    ...
    __usernameTimer = setTimeout(() => {
      ...
      user.checkUsername().then((data) => {
        this.set({
          ...
          __usernameVerifiedMessage: data.message
        });
      }, () => {
        this.set({
	...
	function encapsulateStyles(node) {
		setAttribute(node, "svelte-686499168", "");
	}

	function add_css() {
		var style = createElement("style");
		style.id = 'svelte-686499168-style';
		style.textContent = "[svelte-686499168].login-dialog-container,[svelte-686499168] .login-dialog-container{display:flex;align-items:center}[svelte-686499168].login-dialog-container.layout-compact,[svelte-686499168] .login-dialog-container.layout-compact{justify-content:center;position:fixed;top:0;left:0;bottom:0;right:0;z-index:1000}[svelte-686499168].login-dialog-overlay,[svelte-686499168] .login-dialog-overlay{display:none}[svelte-686499168].login-dialog-container.layout-compact .login-dialog-overlay,[svelte-686499168] .login-dialog-container.layout-compact .login-dialog-overlay{display:block;position:absolute;top:0;left:0;bottom:0;right:0;background-color:rgba(0, 0, 0, 0.3);z-index:1}[svelte-686499168].login-dialog,[svelte-686499168] .login-dialog{display:flex;flex-direction:column}[svelte-686499168].login-dialog-container.layout-compact .login-dialog,[svelte-686499168] .login-dialog-container.layout-compact .login-dialog{padding:2em;box-shadow:0px 5px 36px 0px rgba(0,0,0,0.25);background-color:#fff;max-height:80vh;max-width:80vw;overflow:auto;z-index:2}[svelte-686499168].login-dialog-container.layout-compact .login-dialog.loading,[svelte-686499168] .login-dialog-container.layout-compact .login-dialog.loading{width:90vw;height:90vh;max-width:260px;max-height:100px;justify-content:center;align-items:center}";
	...
	// (38:8) {{#if __clientConfig.allow_registration}}
	...
	function create_if_block_16(state, component) {
		var div, label, span, text_2, input, input_updating = false;
		...
		return {
			c: function create() {
				...
				label = createElement("label");
				span = createElement("span");
				span.innerHTML = "Email <span class=\"pf-required\">*</span>";
				text_2 = createText("\n                ");
				input = createElement("input");
				this.h();
			},
	...
	function create_if_block_25(state, component) {
		...
		return {
			c: function create() {
				...
				text = createText("\n            ");
				a = createElement("a");
				a.textContent = "I can't access my account.";
				this.h();
			},

			h: function hydrate() {
				div.className = "pf-element";
				a.className = "pf-field";
				a.href = "javascript:void(0);";
				addListener(a, "click", click_handler);
			},
	...
	function create_if_block_7(state, component) {
		...
		var if_block = (state.__clientConfig.allow_registration) && create_if_block_8(state, component);
		...
		return {
			...
			p: function update(changed, state) {
				...
				if (state.__clientConfig.allow_registration) {
					if (if_block) {
						if_block.p(changed, state);
					} else {
	...
	function TilmeldLogin(options) {
		...
		if (!document.getElementById("svelte-686499168-style")) add_css();
