...
var TilmeldRecover = (function(Nymph, User) { "use strict";
  ...
  sendRecoveryLink() {
    ...
    User.sendRecoveryLink({'recoveryType': this.get('recoveryType'), 'account': this.get('account')}).then((data) => {
      if (!data.result) {
        this.set({__failureMessage: data.message});
      } else {
        if (this.get('recoveryType') === 'username') {
          this.set({__successRecoveredMessage: data.message});
        } else if (this.get('recoveryType') === 'password') {
          this.set({__hasSentSecret: true});
        }
      }
  ...
  recover() {
    ...
    User.recover({'username': this.get('account'), 'secret': this.get('secret'), 'password': this.get('password')}).then((data) => {
      if (!data.result) {
        this.set({__failureMessage: data.message});
      } else {
        this.set({__successRecoveredMessage: data.message});
  ...
  this.observe('__showDialog', (value) => {
	...
	function encapsulateStyles(node) {
		setAttribute(node, "svelte-2382969747", "");
	}

	function add_css() {
		var style = createElement("style");
		style.id = 'svelte-2382969747-style';
		style.textContent = "[svelte-2382969747].recovery-dialog-container,[svelte-2382969747] .recovery-dialog-container{display:flex;align-items:center}[svelte-2382969747].recovery-dialog-container.layout-compact,[svelte-2382969747] .recovery-dialog-container.layout-compact{justify-content:center;position:fixed;top:0;left:0;bottom:0;right:0;z-index:1000}[svelte-2382969747].recovery-dialog-overlay,[svelte-2382969747] .recovery-dialog-overlay{display:none}[svelte-2382969747].recovery-dialog-container.layout-compact .recovery-dialog-overlay,[svelte-2382969747] .recovery-dialog-container.layout-compact .recovery-dialog-overlay{display:block;position:absolute;top:0;left:0;bottom:0;right:0;background-color:rgba(0, 0, 0, 0.1);z-index:1}[svelte-2382969747].recovery-dialog,[svelte-2382969747] .recovery-dialog{display:flex;flex-direction:column}[svelte-2382969747].recovery-dialog-container.layout-compact .recovery-dialog,[svelte-2382969747] .recovery-dialog-container.layout-compact .recovery-dialog{padding:2em;box-shadow:0px 5px 36px 0px rgba(0,0,0,0.25);background-color:#fff;max-height:80vh;max-width:80vw;overflow:auto;z-index:2}[svelte-2382969747].recovery-dialog-container.layout-compact .recovery-dialog.loading,[svelte-2382969747] .recovery-dialog-container.layout-compact .recovery-dialog.loading{width:90vw;height:90vh;max-width:260px;max-height:100px;justify-content:center;align-items:center}[svelte-2382969747].recovery-dialog-title,[svelte-2382969747] .recovery-dialog-title{padding-top:0;margin-top:0}";
	...
	function create_if_block_10(state, component) {
		var div, text_2, div_1, label, span, text_4, input, input_updating = false, text_7, div_2, label_1, span_1, text_9, input_1, input_1_updating = false, text_12, div_3, label_2, span_2, text_14, input_2, input_2_updating = false;
	...
	function TilmeldRecover(options) {
		...
		if (!document.getElementById("svelte-2382969747-style")) add_css();
